...
void setup() // 設定區
{ // 設定區開頭
  pinMode(ledPin, 0x1); // 設定 ledPin 為 OUTPUT
  pinMode(buttonPin, 0x0); // 設定 buttonPin 為 INPUT
  ...
  digitalWrite(ledPin, 0x1); // 向 ledPin 輸出 HIGH(點亮 LED)
...
{ // 迴圈區開頭
  ...
  if (buttonState == 0x1) // 如果 buttonState 等於 HIGH(按下)
  { // 如果執行區開頭
    ...
    digitalWrite(ledPin, 0x0); // 向 ledPin 輸出 LOW(LED 暗)
